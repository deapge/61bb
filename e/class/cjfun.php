...
function CjNewsIn($classid,$id,$checked,$uptime,$userid,$username){
	...
	while($r=$empire->fetch($sql))
	{
		...
		//4usky.com infopfen,infopfennum 不写入 2011-12-12
		$isql=$empire->query("insert into {$dbtbpre}ecms_".$cr[tbname]."(classid,onclick,newspath,keyboard,keyid,userid,username,ztid,checked,truetime,ismember,dokey,isgood,titlefont,titleurl,filename,groupid,newstempid,plnum,firsttitle,isqf,userfen,totaldown,closepl,havehtml,lastdotime,haveaddfen,votenum,stb,ttid,newstime,ispic".$ifield.") values($cr[newsclassid],0,'$newspath','$r[keyboard]','$keyid',$r[userid],'$r[username]','$cr[newsztid]','$checked','$r[truetime]',0,1,0,'$r[titlefont]','$r[titleurl]','$filename',0,'".$newstempid."',0,0,'$isqf',0,0,0,0,'$r[truetime]',0,0,'$stb',0,'$r[newstime]','$ispic'".$ivalue.");");
		$id=$empire->lastid();
		//副表
...
function CjNewsIn_all($classid,$checked,$uptime,$start,$userid,$username){
	...
	//exit("<b>$cr[classname]</b>&nbsp;&nbsp;".$fun_r['OneCjLoadDbSuccess']."(ID:<font color=red><b>".$newstart."</b></font>)");
...
function EchoUrl($text,$exp,$exp1,$dr,$url,$classid,$num,$checkrnd){
	...
	for($i=1;$i<count($r)&&$i<=$num;$i++)
	{
		...
		//3366直接采集播放页 2011-11-14 http://33666.4usky.com 
		if(strstr($dourl,"61baobao.com"))
		{
			//echo($dourl);
			if(strstr($dourl,"/tingerge/")||strstr($dourl,"/jianggushi/")||strstr($dourl,"/wanyouxi/")||strstr($dourl,"/xuezhishi/"))
			{
				//http://www.61baobao.com/xuezhishi/tangshisongci/20120224/11119.html
				$url_r=explode("/",$dourl);
				$count=count($url_r);
				/*
				 for($i=0;$i<$count;$i++)
				...
				*/
				$dourl=$url_r[0].'/'.$url_r[1].'/'.$url_r[2].'/'.$url_r[3].'/'.$url_r[6].'/';
				$dourl=str_replace('.html','',$dourl);
				//exit($dourl);
		...
		//根据图片判断有没有删除2012-5-29
		if(strstr($titlepic,"default_titlepic.png"))
		{
			continue;
		}
		//将地址写入数据库
...
function ViewEchoUrl($text,$exp,$exp1,$dr,$url,$classid,$num,$checkrnd){
	...
	for($i=1;$i<count($r)&&$i<=$num;$i++)
	{
		...
		{continue;}
		$dourl=addslashes($dourl);
		//3366直接采集播放页 2011-11-14 http://3366.4usky.com 
		if(strstr($dourl,"61baobao.com"))
		{
			//echo($dourl);
			if(strstr($dourl,"/tingerge/")||strstr($dourl,"/jianggushi/")||strstr($dourl,"/wanyouxi/")||strstr($dourl,"/xuezhishi/"))
...
function GetMoreCjPagetextall($self,$newstextzz,$smallpagezz,$pagezz,$pagetype,$firsttext,$the_r){
	...
	for($i=1;$i<$count;$i++)
	{
		...
		//
		//echo(strstr($allpagetext,$newstext)."--");
		if(strstr($allpagetext,$newstext))break;//2012-6-11 4usky
		if($newstext)
		{
			$allpagetext.=$addpagetag.$newstext;
		}
...
function GetNewsInfo($classid,$checkrnd,$start,$userid,$username){
	...
	while($nr=$empire->fetch($sql))
	{
		...
		//判断是不是网页游戏路径 4usky 2012-1-13
		if(strstr($info,'"NeedFrame": "1"'))
			$NeedFrame=1;
		...
		for($c=0;$c<$count-1;$c++)
		{
			...
			if($dofield=="titlepic")
			{
				if($nr[titlepic])
				{
					...
					//$zzvalue=str_replace("_100","_75_round",$nr[titlepic]);
			...
			//---------------------4usky.com  2011-11-11 -----------------
		//标题图片源
			if($dofield=="titlepic_4usky" )
			{
				if($nr[titlepic])
				{
					$zzvalue=$nr[titlepic];
			...
			if($dofield=="gamebigpic" ||$dofield=="gamebigpic_4usky")
			{
				if($nr[titlepic])
				{
					$tt=str_replace("/pic/","/dpic/",$nr[titlepic]); 
					$zzvalue=$tt;
				}
			}

			if($dofield=="flashurl" ||$dofield=="flashurl_4usky")
			{
				if($zzvalue)
				{
					$zzvalue="http://s.61baobao.com".base64_decode(base64_decode(base64_decode($zzvalue)));
					
				}
			}
			/* 
			if($dofield=="save_flasurl")
			{
				if($NeedFrame)
					$zzvalue="";//不远程保存
			}
			...
			if($dofield=="width" )
			{
				$zzvalue=GetGameFeature_3366($zzvalue,4);	
			}
			if($dofield=="height" )
			{
				$zzvalue=GetGameFeature_3366($zzvalue,5);	
			}
			if($dofield=="filesize" )
			{
				$zzvalue=ceil(GetGameFeature_3366($zzvalue,2)/1024);	
			}*/
		//---------------------4usky.com  2011-11-11 ----------------------------------------------
		...
		//exit("insert into {$dbtbpre}ecms_infotmp_".$r[tbname]."(classid,oldurl,checked,keyboard,newstime,truetime,tmptime,userid,username".$ifield.") values($classid,'$nr[newsurl]',0,'".addslashes($keyboard)."','$newstime',$truetime,'$tmptime',$userid,'".addslashes($username)."'".$ivalue.");");
...
function ViewGetNewsInfo($classid,$newspage,$userid,$username){
	...
	//exit("select * from {$dbtbpre}ecms_infoclass_".$r[tbname]." where classid='$classid'");
...
//==================================================
//获取游戏尺寸等特征 2011-11-11  4usky.com
function GetGameFeature_3366($str,$i)
{
	$returnvalue="";
	 
	if(!empty($str))
	{
		//InitMGPFlashPage(29233, "逗小猴子笑2", 2313701, 0, 640,480,0,"冒险", 0,"http://" ,"","44", "|||||", 0, 1, "1270795336");	
		$sarr=explode(",",$str);
		$returnvalue=trim($sarr[$i]);
