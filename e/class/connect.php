...
function page1($num,$line,$page_line,$start,$page,$search){
	//<i class="no_a"><span>上一页</span></i><i class='current'><span>1</span></i><a href="http://www.61baobao.com/donghuapian/index_2.html" class="wrap"><span>2</span></a>><a href="http://www.61baobao.com/donghuapian/index_2.html" class="next_p">下一页</a><a href="http://www.61baobao.com/donghuapian/index_12.html"class="wrap"><span>尾页</span></a>
	...
	$firststr='<i class="no_a"><span>共'.$totalpage.'页</span></i>';
	//上一页
	if($page<>0)
	{
		$toppage='<a href="'.$url.'=0'.$search.'" class="wrap"><span>'.$fun_r['startpage'].'</span></a>';
		$pagepr=$page-1;
		$prepage='<a href="'.$url.'='.$pagepr.$search.'" class="wrap"><span>'.$fun_r['pripage'].'</span></a>';
	}
	//下一页
	if($page!=$totalpage-1)
	{
		$pagenex=$page+1;
		$nextpage='<a href="'.$url.'='.$pagenex.$search.'"class="wrap"><span>'.$fun_r['nextpage'].'</span></a>';
		$lastpage='<a href="'.$url.'='.($totalpage-1).$search.'"class="wrap"><span>'.$fun_r['lastpage'].'</span></a>';
	}
	...
	for($i=$starti;$i<$totalpage&&$no<$page_line;$i++)
	{
		...
		if($page==$i)
		{
			$is_1="<i class='current'><span>";
			$is_2="</span></i>";
		}
		else
		{
			$is_1='<a href="'.$url.'='.$i.$search.'"class="wrap"><span>';
			$is_2="</span></a>";
		}
		$pagenum=$i+1;
...
//建立多级目录 2011-12-19 4usky.com
function mkpath_4usky($mkpath,$mode=0777)
{
...
function ReplaceListVars($no,$listtemp,$subnews,$subtitle,$formatdate,$url,$haveclass=0,$r,$field,$docode=0){
	...
	if(strstr($listtemp,'[!--ztimg--]'))//下拉式
	{
		$listtemp=str_replace('[!--ztimg--]',$r[ztimg],$listtemp);
		$listtemp=str_replace('[!--ztname--]',$r[ztname],$listtemp);
		$listtemp=str_replace('[!--ztpath--]',$r[ztpath],$listtemp);
...
function ReturnTagsHtml($tags)
{
	...
	{
		if($i==count($tagarr)-1)
			$add=$add." tagname='".$tagarr[$i]."'";
		else
			$add=$add." tagname='".$tagarr[$i]."' or ";
	}
	...
	{
		$tagStr=$tagStr."<a name=\"keyWord\" class=\"t_tag\" target=_blank href=/tag/".$link_r[tagdir]."/>".$link_r[tagname]."</a>|";
	}
...
//上传文件
function DoTranFile_4usky($file,$file_name,$file_type,$file_size,$classid,$ecms=0){
	...
	//存放目录
	//$fspath=ReturnFileSavePath($classid);
	$r[savepath]=ECMS_PATH."upload\\".$filepath;

	DoMkdir($r[savepath]);
	//echo($r[savepath ]);
	
	//附件地址
	$r[url]="/upload/".$filepath.$r[filename];
	//缩图文件
	$r[name]=$r[savepath]."small".$r[insertfile];
	//附件文件
	$r[yname]=$r[savepath].$r[filename];
