...
function sys_ShowListMorePage($num,$page,$dolink,$type,$totalpage,$line,$ok,$search=""){
	...
	//$firststr='<a title="Total record">&nbsp;<b>'.$num.'</b> </a>&nbsp;&nbsp;'; 2012-6-15
	$firststr='<i class="no_a"><span>共'.$totalpage.'页</span></i>';
	//上一页
	if($page<>1)
	{
		$toppage='<a href="'.$dolink.'index'.$type.'"class="wrap"><span>'.$fun_r['startpage'].'</span></a>';
		$pagepr=$page-1;
		if($pagepr==1)
		{
			$prido="index".$type;
		...
		$prepage='<a href="'.$dolink.$prido.'" class="wrap"><span>'.$fun_r['pripage'].'</span></a>';
	}
	//下一页
	if($page!=$totalpage)
	{
		$pagenex=$page+1;
		$nextpage='<a href="'.$dolink.'index_'.$pagenex.$type.'" class="next_p"><span>'.$fun_r['nextpage'].'</span></a>';
		$lastpage='<a href="'.$dolink.'index_'.$totalpage.$type.'" class="wrap"><span>'.$fun_r['lastpage'].'</span></a>';
	}
	...
	for($i=$starti;$i<=$totalpage&&$no<$page_line;$i++)
	{
		...
		elseif($i==1)
		{
			$is_1='<a href="'.$dolink.'index'.$type.'" class="wrap"><span>';
			$is_2="</span></a>";
		}
		else
		{
			$is_1='<a href="'.$dolink.'index_'.$i.$type.' "class="wrap"><span>';
			$is_2="</span></a>";
		}
