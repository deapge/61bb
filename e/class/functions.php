...
function ListHtml($classid,$fields,$enews=0,$userlistr=""){
	...
	/*-----------------------------2011-12-18 4usky.com classtag start*/
	 
	//关联类别内容
	/*
	if(strstr($listtemp,'[!--classtag--]'))
	{
		ob_start();
		$file=ECMS_PATH.'d/tag_c'.$selfclassid.'.htm';
		//exit($file);
		readfile($file);
		$ss=ob_get_contents();
		ob_end_clean();
		$listtemp=str_replace('[!--classtag--]',$ss,$listtemp); 
	}
	*/
	/*-----------------------------2011-12-18 4usky.com classtag end*/
	$no=1;
	$ok=0;
	$changerow=1;
	...
	while($k=$empire->fetch($sql))
	{
		...
		if($no%$lencord==0||($num%$lencord<>0&&$num==$no))
		{
			...
			$string=str_replace('[!--pagetag--]',',最新发布',$string);
			WriteFiletext($path,$classlevel.$string);
			$string='';
		}
		$no++;
	}
	//-------------------------------生成其他排序类别列表 2011-5-19 4usky.com 2144a.com-----------------------------------------
	/*
	if($enews==0)//子栏目列表
	{
		...
		$page=ceil($num/$lencord);	
		$query="select ".ReturnSqlListF($mid)." from {$dbtbpre}ecms_".$class_r[$classid][tbname]." where ".$yhadd."classid='$classid' and checked=1 order by onclick desc limit 560";
		$sql=$empire->query($query);
		$listtext=$list_r[1];
		...
		while($k=$empire->fetch($sql))
		{
			...
			if($no%$lencord==0||($num%$lencord<>0&&$num==$no))
			{
				...
				if($pagenum==1)
				{
					$path=$dopath."view".$dotype;
				}
				else
				{
					$path=$dopath."view_".$ok.$dotype;
				}
				...
				$showpage=str_replace('index','view',$showpage);
				$list1=str_replace($bereplistpage,$showpage,$list_r[0]);
				$list2=str_replace($bereplistpage,$showpage,$list_r[2]);
				//多余数据
				if($changerow<=$rownum&&$listtext<>$list_r[1])
				...
				//替换分页数
				$string=str_replace('[!--list.pageno--]',$pagenum,$string);
				$string=str_replace('"sortMode": "new",','"sortMode": "all",',$string);
				$string=str_replace('[!--pagetag--]',',最多人玩',$string);
				$df0=round($k[onclick]/10000); 
				$zz2="/\<p class=\"date\">(.*?)<\/p>/i";
				$string=preg_replace($zz2," <p class=\"renqi\"><i class=\"i_heart h".$df0."\"></i></p>",$string);
		...
		$page=ceil($num/$lencord);	
		$query="select ".ReturnSqlListF($mid)." from {$dbtbpre}ecms_".$class_r[$classid][tbname]." where ".$yhadd."classid='$classid' and checked=1 order by infopfen desc limit 560";
		$sql=$empire->query($query);
		$listtext=$list_r[1];
		...
		while($k=$empire->fetch($sql))
		{
			...
			if($no%$lencord==0||($num%$lencord<>0&&$num==$no))
			{
				...
				if($pagenum==1)
				{
					$path=$dopath."score".$dotype;
				}
				else
				{
					$path=$dopath."score_".$ok.$dotype;
				}
				...
				$showpage=str_replace('index','score',$showpage);
				$list1=str_replace($bereplistpage,$showpage,$list_r[0]);
				$list2=str_replace($bereplistpage,$showpage,$list_r[2]);
				//多余数据
				if($changerow<=$rownum&&$listtext<>$list_r[1])
				...
				//替换分页数
				$string=str_replace('[!--list.pageno--]',$pagenum,$string);
				$string=str_replace('"sortMode": "new",','"sortMode": "score",',$string);
				$string=str_replace('[!--pagetag--]',',评分最高',$string);
...
function GetHtml($add,$fields,$doall=0){
	...
	//2012-5-28 4usky.com qq:1936240--92game.net
	$cr=$empire->fetch1("select classpagekey,intro from {$dbtbpre}enewsclass where classid='$add[classid]'");
	$pagekey=str_replace('[!--title--]',$pagetitle,$cr[classpagekey]);
	$pagedes=str_replace('[!--title--]',$pagetitle,$cr[intro]);
	$newstemptext=Info_ReplaceSvars($newstemptext,$url,$add[classid],$pagetitle,$pagekey,$pagedes);

	//exit("mid:".$mid);
	//exit($class_r[$add[classid]][classname]);
	if($mid==5)
	{
		$needplayhtm=0;
	}
	else
	{
		...
		//获取播放下载页模板-------------------------------------------------------------4usky.com
		$classname=$class_r[$add[classid]][classname];
		if(strstr($classname,'动画')||strstr($classname,'舞蹈') )//dh
		{
			$PlayTemp_r=GetNewsTemp("12");
			$isdh=1;
		}
		else
		{
			$PlayTemp_r=GetNewsTemp("10");
			$isdh=0;
		}
		$PlayTemptext=$PlayTemp_r[temptext];
		$PlayTemptext=Info_ReplaceSvars($PlayTemptext,$url,$add[classid],$pagetitle,$add[keyboard],$pagetitle);

		//$DownTemp_r=GetNewsTemp("8");
		//$DownTemptext=$DownTemp_r[temptext];
		//$DownTemptext=Info_ReplaceSvars($DownTemptext,$url,$add[classid],$pagetitle,$add[keyboard],$pagetitle);

	
	//if ($public_r['add_havebox'])
	//{
		
	//	$boxTemp_r=GetNewsTemp("12");
	//	$boxTemptext=$boxTemp_r[temptext];
	//	$boxTemptext=Info_ReplaceSvars($boxTemptext,$url,$add[classid],$pagetitle,$add[keyboard],$pagetitle);
	//}
	//获取播放下载页模板-------------------------------------------------------------4usky.com

	//相关信息 2011-11-21 3366.4usky.com
	$aboutlist=array();
	$aboutlist=GetGameFromTags($add[infotags],$add[id],60);//60为介绍，播放，下载页相关游戏的最大值


	if(strstr($newstemptext,'[!--aboutgames--]'))
	{
		if($add[infotags]){
    		$aboutgames=GetAboutGame($aboutlist,0,8);
		}else{
			$aboutgames=$fun_r['NotLinkNews'];}
	}	
	
	if(strstr($newstemptext,'[!--othergames--]'))
	{
		if($add[infotags]){
    		$othergames=GetAboutGame($aboutlist,1,7);
		}else{
			$othergames=$fun_r['NotLinkNews'];}
	}
	...
	for($i=1;$i<$fcount;$i++)
	{
		...
		//----------------------------------------------------------4usk y.com
		if($needplayhtm)
			$PlayTemptext=str_replace('[!--'.$f.'--]',$value,$PlayTemptext);
		 
		//$DownTemptext=str_replace('[!--'.$f.'--]',$value,$DownTemptext);
		//if ($public_r['add_havebox'])
			//$boxTemptext=str_replace('[!--'.$f.'--]',$value,$boxTemptext);
		//------------------------------------------------------------4 u s ky.com
	}
	...
	//if ($public_r['add_havebox'])
	//		$boxTemptext=str_replace('[!--boxflashurl--]',$boxflashurl,$boxTemptext);
	//固定变量
	...
	//$newstempstr=str_replace('[!--class.url--]',$class_r[$add[classid]][classurl],$newstempstr);
	...
	*/
	...
	//$newstempstr=str_replace('[!--othergames--]',$othergames,$newstempstr); 
	$newstempstr=str_replace('[!--aboutgames--]',$aboutgames,$newstempstr);
	

	//------------------------------------------------------------4us ky.c om 2011-4-12
	$taghtml=ReturnTagsHtml($add[infotags]);
	
	$newstempstr=str_replace('[!--infotags--]',$taghtml,$newstempstr);
		 
	$opkeystr=$add[operation];
															
	//if(!empty($opkeystr))
	//{
	//	$opkeystr="".$opkeystr."</dd>";
	//}
	$newstempstr=str_replace('[!--opkey--]',$opkeystr,$newstempstr);
	 

	/*2011-12-13
	//关联类别内容
	if(strstr($newstemptext,'[!--classabout--]'))
	{
		ob_start();
		$file=ECMS_PATH.'d/intro_c'.$add[classid].'.htm';
		readfile($file);
		$string=ob_get_contents();
		ob_end_clean();
		$newstempstr=str_replace('[!--classabout--]',$string,$newstempstr);
		$PlayTemptext=str_replace('[!--classabout--]',$string,$PlayTemptext);
	}
	*/
	//------------------------------------------------------------4usky.com
	//链接
	...
	//2012-6-6 4usky.com
	for($t=1;$t<=$add[volumenum];$t++)
	{
		$titleurl2=str_replace('.html','',$titleurl);
		$titleurl2=str_replace('.htm','',$titleurl2);
		$onlinepath=$onlinepath."<li><a href=\"".$titleurl2."/".$t.$filetype."\" id=".$t.">".$add[title]."第".$t."集</a></li>";
	}
	//exit($onlinepath);
	$onlinepathhtm="<div class=\"cartoonList\"><h2>剧集列表</h2><ul>";
	$onlinepathhtm2="<div class=\"c\"></div></ul></div>";

	if($onlinepath)
	{
		$onlinepathintro=$onlinepathhtm.$onlinepath.$onlinepathhtm2;
	}


	$newstempstr=str_replace('[!--onlinepath--]',$onlinepathintro,$newstempstr);
	$PlayTemptext=str_replace('[!--onlinepath--]',$onlinepath,$PlayTemptext);
	$newstempstr=str_replace('[!--titleurl--]',$titleurl,$newstempstr);

	if($isdh)//2012-6-6
		$playurl=$add[id]."/"."1".$filetype;
	else
		$playurl=str_replace($add[filename].$filetype,$add[filename]."_play".$filetype,$titleurl);

	$newstempstr=str_replace('[!--playurl--]',$playurl,$newstempstr);
	...
	if(strstr($newstemptext,'[!--info.next--]'))
	{
		...
		if(empty($next_r[id]))
		{
			$infonext="<a href='../'>".$fun_r['HaveNoNextLink']."</a>";
		}
		else
		{
			//链接
		...
		$PlayTemptext=str_replace('[!--info.next--]',$infonext,$PlayTemptext);
	}
	//上一篇
	...
	if(strstr($newstemptext,'[!--pinfopfen--]'))
	{
		...
		if($needplayhtm)
			$PlayTemptext=str_replace('[!--pinfopfen--]',$pinfopfen,$PlayTemptext);
	...
	{
		//---------------------------------------------------------4usky.com playpage----------//
		$PlayTemptext=str_replace('[!--id--]',$add[id],$PlayTemptext);
		$PlayTemptext=str_replace('[!--classid--]',$add[classid],$PlayTemptext);
		$PlayTemptext=str_replace('[!--class.name--]',$class_r[$add[classid]][classname],$PlayTemptext);
		$PlayTemptext=str_replace('[!--title--]',$add[title],$PlayTemptext);

		$classurl=sys_ReturnBqClassname($add,9);
		$PlayTemptext=str_replace('[!--class.url--]',$classurl,$PlayTemptext);

		$PlayTemptext=str_replace('[!--plnum--]',$add[plnum],$PlayTemptext);
		$PlayTemptext=str_replace('[!--ttid--]',$add[ttid],$PlayTemptext);
		$PlayTemptext=str_replace('[!--tt.name--]',$class_tr[$add[ttid]][tname],$PlayTemptext);
		$PlayTemptext=str_replace('[!--onclick--]',$add[onclick],$PlayTemptext);
		$PlayTemptext=str_replace('[!--titlepic--]',$add[titlepic],$PlayTemptext);
		$PlayTemptext=str_replace('[!--userid--]',$add[userid],$PlayTemptext);
		$PlayTemptext=str_replace('[!--other.link--]',$keyboardtext,$PlayTemptext);
		$PlayTemptext=str_replace('[!--news.url--]',$public_r[newsurl],$PlayTemptext);
		$PlayTemptext=str_replace('[!--totaldown--]',$add[totaldown],$PlayTemptext);
		$PlayTemptext=str_replace('[!--keyboard--]',$add[keyboard],$PlayTemptext);
		$PlayTemptext=str_replace('[!--info.pre--]',$infonext,$PlayTemptext);
		$PlayTemptext=str_replace('[!--infopfen--]',$add[infopfennum],$PlayTemptext);
		$PlayTemptext=str_replace('[!--infopfennum--]',$add[infopfennum],$PlayTemptext);
		$PlayTemptext=str_replace('[!--infotags--]',$taghtml,$PlayTemptext);
	
	/*
		if(strstr($add[flashurl],'.htm')||strstr($add[flashurl],'.html'))
		{
			$PlayTemptext=str_replace('[!--needframe--]',1,$PlayTemptext);
		}
		else
		{
			$PlayTemptext=str_replace('[!--needframe--]',0,$PlayTemptext);
		}
*/
	/*
		$DownTemptext=str_replace('[!--classid--]',$add[classid],$DownTemptext);
		$DownTemptext=str_replace('[!--class.name--]',$class_r[$add[classid]][classname],$DownTemptext);
		$DownTemptext=str_replace('[!--onclick--]',$add[onclick],$DownTemptext);
		$DownTemptext=str_replace('[!--titleurl--]',$titleurl,$DownTemptext);
		$DownTemptext=str_replace('[!--news.url--]',$public_r[newsurl],$DownTemptext);
		
		$DownTemptext=str_replace('[!--infopfen--]',$jinquepfen,$DownTemptext);
		$DownTemptext=str_replace('[!--infotags--]',$taghtml,$DownTemptext);

		if($public_r['add_havebox'])
		{
			$boxTemptext=str_replace('[!--onclick--]',$add[onclick],$boxTemptext);
			$boxTemptext=str_replace('[!--infopfennum--]',$add[infopfennum],$boxTemptext);
			$boxTemptext=str_replace('[!--class.name--]',$class_r[$add[classid]][classname],$boxTemptext);
			$boxTemptext=str_replace('[!--infotags--]',$add[infotags],$boxTemptext);
			$boxTemptext=str_replace('[!--pinfopfen--]',$pinfopfen,$boxTemptext); 
		}
		*/
		//链接
		$titleurl=sys_ReturnBqTitleLink($add);
		$PlayTemptext=str_replace('[!--titleurl--]',$titleurl,$PlayTemptext);
		$playurl=str_replace($add[filename].$filetype,$add[filename]."_play".$filetype,$titleurl);
		//$downurl=str_replace($add[filename].$filetype,$add[filename]."_down".$filetype,$titleurl);
		$PlayTemptext=str_replace('[!--playurl--]',$playurl,$PlayTemptext);
		$PlayTemptext=str_replace('[!--page.stats--]',$onclick,$PlayTemptext);
		//$PlayTemptext=str_replace('[!--smalltext--]',$add[smalltext],$PlayTemptext);
		//$PlayTemptext=str_replace('[!--opkey--]',$opkeystr,$PlayTemptext);
		//$DownTemptext=str_replace('[!--opkey--]',$opkeystr,$DownTemptext);
		

		/*
		if(strstr($PlayTemptext,'[!--aboutgames--]'))
		{
			if($add[infotags]){
				$playaboutgames=GetAboutGame($aboutlist,0,8);
			}else{
				$playaboutgames=$fun_r['NotLinkNews'];}
		}	
		 
		*/

		//$PlayTemptext=str_replace('[!--othergames--]',$othergames,$PlayTemptext); 
		//$PlayTemptext=str_replace('[!--aboutgames--]',$aboutgames,$PlayTemptext);

		/*if(strstr($DownTemptext,'[!--downaboutfont--]'))
		{
			if($add[infotags]){
				$downaboutgames=GetDownAboutGame($aboutlist,8);
			}else{
				$downaboutgames=$fun_r['NotLinkNews'];}
		}
		$DownTemptext=str_replace('[!--downaboutfont--]',$downaboutgames,$DownTemptext);

		if(strstr($DownTemptext,'[!--downaboutpic--]'))
		{
			if($add[infotags]){
				$downaboutgames=GetDownAboutGamePic($aboutlist,3);
			}else{
				$downaboutgames=$fun_r['NotLinkNews'];}
		}
		$DownTemptext=str_replace('[!--downaboutpic--]',$downaboutgames,$DownTemptext);
	*/
		//-----------------------------------------------------4usky.com playpage----------//
	...
	if($pf&&strstr($add[$pf],$expage))//有分页
	{
		...
		//2012-6-11exit('jjkjk'.$thispagenum);
		//取得分页
	...
	else
	{
		...
		if($needplayhtm)
		{
			...
			if($isdh)//特殊儿童舞蹈类播放地址
			{
				//$("#moviesbox").html(addFlash('aHR0cDovL3BsYXllci55b3VrdS5jb20vcGxheWVyLnBocC9zaWQvWE1qYzFNekl5TVRNeS92LnN3Zg=='))
				//initData.flashUrl="aHR0cDovL3BsYXllci55b3VrdS5jb20vcGxheWVyLnBocC9zaWQvWE1qYzFNekl5TVRNeS92LnN3Zg==";init();
				if(!strstr($cjmoviepath,','))
				{
					
					$PlayTemptext=str_replace('[!--playaddr--]','initData.flashUrl="'.$add[cjmoviepath].'";init();',$PlayTemptext);
				}
				else
				{
					$PlayTemptext=str_replace('[!--playaddr--]','$("#moviesbox").html(addFlash('.$add[cjmoviepath].'));',$PlayTemptext);
				}
			}
			*/
			//echo('dsds'.$isdh);
			if($isdh)
			{
				DoMkdir($doclasspath.$newspath."/".$add[filename]."/");
				
					$path_r=explode("|||",$add[cjmoviepath]);
					for($t=1;$t<=$add[volumenum];$t++)
					{
						$playtext=$PlayTemptext;
						$p=$path_r[$t-1];
						$p=str_replace(',','\',\'',$p);
						//$playtext=str_replace('[!--volumeid--]',$t,$playtext);

						if(!strstr($add[cjmoviepath],','))//特殊儿童舞蹈类播放地址
							$p='initData.flashUrl="'.$p.'";init();';
						else
							$p='$("#moviesbox").html(addFlash(\''.$p.'\'));';

						$playtext=str_replace('[!--playaddr--]',''.$p.'',$playtext);
						$playtext=str_replace('[!--volumeno--]',$t,$playtext);
						//$playtext=str_replace('[!--titleurl--]',$titleurl,$playtext);
						$playfile=$doclasspath.$newspath."/".$add[filename]."/".$t.$filetype;
						//echo($playfile."<br>");
						WriteFiletext($playfile,$addlevel.$playtext);
					} 
			...
			{
				//echo('erge');
				$playfile=$doclasspath.$newspath.$add[filename]."_play".$filetype;
				WriteFiletext($playfile,$addlevel.$PlayTemptext);
			}
		}
		//---------------------------------------------------------4usky.com-------//
...
function GetAboutGame($aboutlist,$startpage,$getnum){
	$link_r=array();
	$link_r=array_slice($aboutlist,$startpage,$getnum);
	$ii=0;
	$aboutstr="";
	//if($startpage==0)
	//{
		foreach( $link_r as $value)
		{
			if($ii<$getnum)
			{
				$titleurl=sys_ReturnBqTitleLink($value);
				

				$aboutstr=$aboutstr."<li rel=\"$value[id]\" ><a href=\"$titleurl\" target=\"_blank\" title=\"$value[title]\"><img src=\"$value[titlepic]\" alt=\"$value[title]\"/><span class=\"txt\">$value[title]</span></a></li>";		
			}			
			$ii=$ii+1;			
		}	
	...
	else
	{
		foreach( $link_r as $value)
		{
			...
			{
				$titleurl=sys_ReturnBqTitleLink($value);
				$aboutstr=$aboutstr."<li><a  href=\"$titleurl\" target=\"_blank\"><img src=\"$value[titlepic]\" width=\"80\" height=\"60\" title=\"$titleurl\" /></a><span><a href=\"$titleurl\" target=\"_blank\" title=\"$value[title]\">$value[title]</a></span></li>";
			}			
			$ii=$ii+1;			
		}	
	}
...
//根据标签获取相关信息 2010-11-17
function GetGameFromTags($tags,$id,$getnum){
	global $empire,$public_r,$class_r,$fun_r,$dbtbpre;	
	$list = $singlelist = $returnlist = array();
	if($tags)
	{	
		$tagr=explode(",",$tags);
		$count=count($tagr);
		//echo($count."<br>");
		$ii=1;
		for($i=0;$i<$count;$i++) //从二个开始
		{
			//if ($i==($count-1))
			//{
				$add2=" infotags like '%".$tagr[$i]."%' and id<>".$id." order by onclick desc limit ".$getnum;
			//}
			//else
			//{
				//$add2=" infotags like '%".$tagr[$i]."%' and id<>".$id." order by onclick desc limit 12";
			//}
			$sql2="select id,newspath,newstime,title,filename,titleurl,groupid,classid,titlepic from {$dbtbpre}ecms_game where ".$add2."";
		...
		$singlelist = a_array_unique($list);	
		$returnarray=array_slice($singlelist,0,$getnum);
	}

	return $returnarray;
}
//除去array里重复的项 2010-11-18                                                                                                     
