...
//---------------------------------------------------------------------4usky.com
function AddPl_flash34($username,$password,$nomember,$key,$saytext,$id,$classid,$repid,$add){
	...
	//是否关闭评论
	$r=$empire->fetch1("select classid,closepl from {$dbtbpre}ecms_".$class_r[$classid][tbname]." where id='$id' and classid='$classid'");
	//exit("select classid,closepl from {$dbtbpre}ecms_".$class_r[$classid][tbname]." where id='$id' and classid='$classid'".$r[classid]);
	if(empty($r[classid]))
	{printerror("ErrorUrl","history.go(-1)",1);}
	if($class_r[$r[classid]][openpl])
	{printerror("CloseClassPl","history.go(-1)",1);}
	...
	if($repid)
	{
		$repr=$empire->fetch1("select replyid from {$dbtbpre}enewspl where plid=$repid and replyid<>'' and replyid<>'0' limit 1");
		if(!empty($repr[replyid]))
		{
			
			$repid=$repr[replyid].','.$repid;
			$reparr=explode(',',$repid);
			$anscount=count($reparr)+1;
	...
	$ret_r=ReturnPlAddF($add,$pr,0);
	//主表
	$sql=$empire->query("insert into {$dbtbpre}enewspl(username,sayip,saytime,id,classid,checked,zcnum,fdnum,userid,isgood,stb,anscount,replyid) values('".$username."','$sayip','$saytime','$id','$classid','$checked',0,0,'$muserid',0,'$pr[pldeftb]',$anscount,'$repid');");
	$plid=$empire->lastid();
	//副表

	
	$ip=ip2int($sayip);
	$sql="select IP_Province,IP_City from {$dbtbpre}4usky_iptocity where IP_Start<='$ip' order by IP_Start desc limit 1";
	$nr=$empire->fetch1($sql);
	$sheng=$rs["IP_Province"];
	$city=$rs["IP_City"];
	$from=$nr["IP_Province"].$nr["IP_City"];


	//exit("insert into {$dbtbpre}enewspl_data_".$pr['pldeftb']."(plid,classid,id,icon,score,from,saytext".$ret_r['fields'].") values('$plid','$classid','$id','$icon','$score','$from','".addslashes($saytext)."'".$ret_r['values'].");");

	$fsql=$empire->query("insert into {$dbtbpre}enewspl_data_".$pr['pldeftb']."(plid,classid,id,icon,score,fromwhere,saytext".$ret_r['fields'].") values('$plid','$classid','$id','$icon','$score','$from','".addslashes($saytext)."'".$ret_r['values'].");");


	/*-----------------------------------2012-3-7 3366评论不评分---------*/
	//if($score<1){$score=1;}
	//if($score>10){$score=10;}
	//$sql=$empire->query("update {$dbtbpre}ecms_".$class_r[$classid]['tbname']." set infopfen=infopfen+$score,infopfennum=infopfennum+1 where id=$id ");
	//if($sql)
	//{
	//		$nr=$empire->fetch1("select infopfen,infopfennum from {$dbtbpre}ecms_".$class_r[$classid]['tbname']." where id='$id'");
	...
	if($sql)
	{
		...
		echo("<script type=\"text/javascript\">document.domain=\"".$public_r[add_domain]."\";parent.Module.Comment._onSubmitSucc({\"result\":1,\"resultstr\":\"success\",\"uin\":0,\"type\":0});</script>");
	}
	else
	{//printerror("DbError","history.go(-1)",1);
	}
...
//---------------------------------------------------------------------
//新添加反对评论-------------4usky.com
function DoForPl_4usky($add){
	global $empire,$dbtbpre;
	$classid=(int)$add['classid'];
	//$id=(int)$add['id'];
	$plid=(int)$add['plid'];
	$action=(int)$add['action'];
	//$doajax=(int)$add['doajax'];
	...
	if(getcvar('lastforplid'.$plid))
	{
		...
		//$doajax==1?ajax_printerror('','','ReDoForPl',1):printerror('ReDoForPl','',1);
		//exit();
		exit("var gMgpAddGoodResult = {\"result\":13006,\"goodcount\":0,\"uin\":0};");
	}
	if($action==2)
	{
		$f='zcnum';
	...
	$sql=$empire->query("update {$dbtbpre}enewspl set ".$f."=".$f."+1 where plid='$plid' ");
	if($sql)
	{
		esetcookie('lastforplid'.$plid,$plid,time()+30*24*3600);	//最后发布
		$sql2="select zcnum from {$dbtbpre}enewspl where plid='$plid'";
		$nr=$empire->fetch1($sql2);

		echo("var gMgpAddGoodResult = {\"result\":0,\"goodcount\":".$nr[zcnum].",\"uin\":0};");
	}
